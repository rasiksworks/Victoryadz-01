 ...
 */
function __OriginkitBase_InfiniteGallery(props: InfiniteGalleryProps) {
    ...
    useEffect(() => {
        ...
        const project = () => {
            ...
            // Perfect velvet crossfade: Keeps tiles 100% solid and crisp, cross-fading only during octave transitions
            const alphaCurrent = frac < 0.65 ? 1 : Math.max(0, (1 - frac) / 0.35);
            const alphaNext = frac > 0.35 ? Math.min(1, (frac - 0.35) / 0.65) : 0;
...
export default function InfiniteGallery(props: InfiniteGalleryProps) {
  return <__OriginkitBase_InfiniteGallery {...(__originkitPresetProps as any)} {...props} />;
}
