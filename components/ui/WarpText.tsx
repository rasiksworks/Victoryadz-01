...
void main() {
  ...
  vec2 displacedText = displaced;
  vec2 splitDir = ambient + pointerWarp;
  float splitLen = length(splitDir);
  splitDir = splitLen > 0.00001 ? splitDir / splitLen : vec2(0.7071, 0.7071);
  vec2 split = splitDir * uRefraction * 0.16 * (0.35 + lens * 1.65);

  vec4 base = sampleText(displacedText);
  float r = sampleText(displacedText + split).r;
  float g = base.g;
  float b = sampleText(displacedText - split).b;
  float a = max(max(sampleText(displacedText + split).a, base.a), sampleText(displacedText - split).a);
...
const buildTextCanvas = ({ container, width, height, dpr, props }: BuildCanvasProps) => {
