...
class MorphEngine {
  ...
  constructor(container: HTMLElement, { items, startIndex, reducedMotion, getOptions, onIndexChange, dprCap }: MorphEngineOptions) {
    ...
    this.renderer = new Renderer({
      alpha: false,
      antialias: true,
      dpr: typeof window !== 'undefined' ? Math.min(window.devicePixelRatio || 1, dprCap) : 1
    });
    ...
    this.program = new Program(this.gl, {
      ...
      uniforms: {
        ...
        uProgress: { value: 0 },
        uDir: { value: 1 },
        uMode: { value: TRANSITIONS[opts.transition as MorphTransition] ?? 0 },
  ...
  syncOptions() {
    const opts = this.getOptions();
    this.program.uniforms.uMode.value = TRANSITIONS[opts.transition as MorphTransition] ?? 0;
